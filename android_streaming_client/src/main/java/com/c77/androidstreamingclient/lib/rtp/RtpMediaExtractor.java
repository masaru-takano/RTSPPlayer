...
//        header_sps = decodeBase64("Z0LAKY1oFB8gHhEI1A==");
//        header_pps = decodeBase64("aM4BqDXI");
//
//        log.debug("***** SPS: " + toString(header_sps));
//        log.debug("***** PPS: " + toString(header_pps));
    ...
    private String toString(final byte[] array) {
        StringBuilder builder = new StringBuilder("{");
        for (int i = 0; i < array.length; i++) {
            if (i > 0) {
                builder.append(", ");
            }
            String hex = toString(array[i]);
            builder.append(hex);
        }
        builder.append("}");
    ...
    private String toString(final byte b) {
        String hex = Integer.toString(0xFF & b, 16);
        if (hex.length() == 1) {
            hex = "0" + hex;
        }
        return "0x" + hex;
